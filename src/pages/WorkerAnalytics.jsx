  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (workerData.length > 0) {
        const matchingWorker = authUser ? workerData.find(w => w.name.toLowerCase() === authUser.name.toLowerCase()) : null;
        const nextId = matchingWorker ? String(matchingWorker.id) : String(workerData[0].id);
