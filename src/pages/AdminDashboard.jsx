...
export default function AdminDashboard() {
  ...
  // Real-time subscription — always re-read from localStorage for cross-tab sync
  useEffect(() => {
    const refreshFromStorage = async () => {
      const [latestWorkers, latestClaims] = await Promise.all([
        base44.entities.Worker.list(),
        base44.entities.Claim.list('-created_date', 50),
      ]);
      setWorkers(latestWorkers);
      setClaims(latestClaims);
      checkForRingAlerts(latestClaims);
    };

    const unsubWorkers = base44.entities.Worker.subscribe(() => {
      refreshFromStorage();
    });

    const unsubClaims = base44.entities.Claim.subscribe(() => {
      refreshFromStorage();
    });
